//----------------------------------------------------------------------------------------------------| MAT von Dietmar Schrausser 2009, 2010    
//																										calc_2 klammerebenen, berechnungsprozedur via calc_1.exe
#include "MAT.h"
#include "MAT3.h"
#include <process.h>
...
main(int argc, char *argv[])
{
	...
	if(fopen ("calc_1.exe","r")==NULL){printf("\ncalc_1.exe .0\n"); exit(0);}//----------------------> calc_1.exe prüfung
	...
	{
		//_spawnlp( _P_NOWAIT, "acrobat.exe", "acrobat.exe", "MAT_Funktionsindex.pdf", NULL );
		system("start MAT_Funktionsindex.pdf");	
		exit(0);
	}
	...
	//funktions matrix schleifen entwicklung (langsame systematik via MAT selbstaufruf)
	if (strcmp(argv[1], "_F_") == 0 )//--------------------------------------------------------------> Funktionsmatrix
	//                                                                                               // bsp _F_ 0 10 ih2
	{	 
		FILE *tmp;

		int jLauf;
		char frm[100], frm_1[100],ind[10];
		
		tmp = fopen( "MAT_fn.txt", "w" );fclose(tmp); //MAT_fn.txt löschen

		                                strcpy (frm,"MAT3 ");
		for(iLauf=4;iLauf<argc;iLauf++) strcat(frm,argv[iLauf]);
		                                strcat(frm,">> MAT_fn.txt");//MAT funktionsbefehl zusammenbau bsp "MAT3 ih2 >> MAT_fn.txt"
		
		for(jLauf=atoi(argv[2]);jLauf<=atoi(argv[3]);jLauf++) //über intervall [a,b]
		{
			itoa(jLauf,ind,10);

			strcpy(frm_1, "MAT i="); //xi definition bsp. "MAT i=1 >nil"
			strcat(frm_1,ind);
			strcat(frm_1," >nil");
			system(frm_1);

			system(frm);//MAT funktionsbefehl
		}
	...
	laenge = strlen(formel);//-----------------------------------------------------------------------> bestimmung der formel zeichensatzlänge
	...
	{ 	
		...
		if(dg_zlr < klm_auf_zlr) while(index <= laenge)//--------------------------------------------> klammerlösungsschleife
		{
			...
			if(arg_[index] == '(')//-----------------------------------------------------------------> bei auf_klammer  
			{
				...
				//	fn_calc_1(tmp_clc); //-----------------------------------------------------------> Subformelübergabe calc_1.h 
